const CONFIG = {
    ...
    youtubeUrl: "https://www.youtube.com/watch?v=94fr3pxcgLs",
    ...
    get textos() {
        ...
        return {
            cat2: { 
                ...
                c: tObj.cat2_c || "Trabajo cada día en mi preparación física y mental, aplicando una mentalidad de cero miedo y perfeccionando cada brazada para dejar a México en lo más alto" 
            },
            cat3: { 
                t: tObj.cat3_t || "VIDEOS DE MI TRAYECTORIA", 
                c: tObj.cat3_c || "Mi mayor fortaleza es la resiliencia mental combinada con la disciplina absoluta en cada competencia" 
            }
        };
    }
};
    ...
    if (cat === 'cat3' || cat === 'cat2') {
        ...
        if (cat === 'cat3') {
            ...
            videoContainer.innerHTML = `
                ...
                <a href="https://www.youtube.com/shorts/Tceb79TluDY" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_tributo_tri || 'Campeona Mundial'}</a>
                <a href="https://www.youtube.com/shorts/IfNAIGBJsBc" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_junto_mar || 'Llegando a México'}</a>
                <a href="https://www.youtube.com/shorts/jkZ2E7Zba2k" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_con_gente || 'Poniendo a México en Alto'}</a>
                <a href="https://www.youtube.com/shorts/QrPvXP3XRv4" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_cantando_beatles || 'Orgullo Mexicano'}</a>
            `;
        } else if (cat === 'cat2') {
            const lang = localStorage.getItem('selectedLanguage') || 'es';
            const tObj = (typeof translations !== 'undefined' && translations[lang]) ? translations[lang] : {};
            ...
            videoContainer.innerHTML = `
                <a href="https://www.youtube.com/shorts/7ra63QVVDt0" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #b00; color: #fff; padding: 14px 25px; border-radius: 12px; text-decoration: none; font-weight: 900; font-size: 0.9rem; text-align: center;">
